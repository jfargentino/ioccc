...
void next0(void) { x ^= x << A; x ^= x >> B; x ^= x << C; y = x * M; } // X1
void next1(void) { x ^= x >> A; x ^= x << B; x ^= x >> C; y = x * M; } // X3
void next2(void) { x ^= x << C; x ^= x >> B; x ^= x << A; y = x * M; } // X2
void next3(void) { x ^= x >> C; x ^= x << B; x ^= x >> A; y = x * M; } // X4
void next4(void) { x ^= x << A; x ^= x << C; x ^= x >> B; y = x * M; } // X5
void next5(void) { x ^= x >> A; x ^= x >> C; x ^= x << B; y = x * M; } // X6
void next6(void) { x ^= x >> B; x ^= x << A; x ^= x << C; y = x * M; } // X7
void next7(void) { x ^= x << B; x ^= x >> A; x ^= x >> C; y = x * M; } // X8
#else
